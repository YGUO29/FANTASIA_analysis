% sync
datafolder = 'C:\Users\slab\Desktop\CaImgProc\testdata';
imgfolder = 'C:\Users\slab\Desktop\CaImgProc\testdata\processed';
...
for i = 1:length(imgfolderlist)
    ...
    mov = fread(fig,Ly*Lx*matadata.numframe,'*uint8');
    mov = reshape(mov, Ly,Lx,matadata.numframe);
    fclose(fig);
...
imgfolderlist = dir(fullfile(imgfolder,'*_reg.bin'));
%
SVDROIPara.useGPU                 = 1; % if you can use a GPU in matlab this accelerate registration approx 3 times
SVDROIPara.sig                    = 0.8;  % spatial smoothing length for clustering; encourages localized clusters
SVDROIPara.nSVDforROI             = 1000; % Number of principal components to keep
SVDROIPara.NavgFramesSVD          = 4000; % how many (binned) timepoints to do the SVD based on
SVDROIPara.niterclustering        = 50;   % how many iterations of clustering
SVDROIPara.ShowCellMap            = 1;
SVDROIPara.Nk0                    = 800;  % how many clusters to start with
SVDROIPara.Nk                     = 400;  % how many clusters to end with
SVDROIPara.resultsavepath = datafolder;
SVDROIPara.minarea = 50; % minimal cell size
SVDROIPara.maxarea = 500; % maximal cell size
for i = 1:length(imgfolderlist)
    i
    filename = strtrim(imgfolderlist(i).name(1:end-8))
    ...
    tic
    [SVDROIPara, U, Sv] = get_svdForROI_v2(SVDROIPara, RegPara, matadata);
